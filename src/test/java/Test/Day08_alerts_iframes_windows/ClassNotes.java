...
How do we handle Select dropdown?
	-> Select drop downs are different from other drop downs.
	...
	-> Then using the object, we can do action on select drop downs.
	-> Select class is coming from Selenium.
How many types of selecting options regarding Select drop downs?
 	-> 3 options
 		- selectByVisibleText
 		- selectByValue
 		- selectByIndex
...
JAVAFAKER:
	-> Java faker is not coming from java, selenium or test ng.
	-> It is another library by itself. Its java faker library.
...
ALERTS
	...
	HOW DO WE HANDLE ALERTS?
		...
		2- Use the instance of alert, to accept, dismiss, or send keys.
			alert.accept();
			alert.dismiss();
